...
#         break
num_entries_in_past = int(input("how far back (number)?"))
...
ts = TradingStrategy(market, num_entries_in_past)
md = MarketDataInterface(market)

while True:
    price = Decimal(str((md.get_ticker_data(False))[3]))
    action = ts.get_final_strategy(name_csvfile)
    print(action)
    ...
    if market[-3:] in ['usd', 'eur']:
        if abs(amount * price) < constants.EUR_USD_MIN:
            action['action'] = 'wait'
            amount = Decimal('0')
            logging.info('do not have minimum amount ({}) to trade'.format(constants.EUR_USD_MIN))
    elif market[-3:] == 'btc':
        if abs(amount * price) < constants.BTC_MIN:
            action['action'] = 'wait'
            amount = Decimal('0')
