    ...
    def get_ticker_data(self, previous):
        ...
        if previous is True:
            return utils.sql_fetch(
                'SELECT * FROM ticker WHERE pair="{pair}" ORDER BY id LIMIT 2'.format(pair=self.market)
            )[1]
        else:
            return utils.sql_fetch(
                'SELECT * FROM ticker WHERE pair="{pair}" ORDER BY id LIMIT 1'.format(pair=self.market)
            )[0]
